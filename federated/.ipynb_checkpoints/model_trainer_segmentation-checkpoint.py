    ...
    def train(self, train_data, device, args):
        ...
        for epoch in range(args.wk_iters):
            ...
            for batch_idx, (x, labels) in enumerate(train_data):
                ...
                # # Before backpropagation, print current parameters
                # print(f"Epoch {epoch} Batch {batch_idx} - Before Backpropagation:")
                # for name, param in model.named_parameters():
                #     print(f"Parameter '{name}' value mean: {param.mean().item()}")
                    
                loss.backward()
                # 打印每个参数的梯度平均值
                for name, param in model.named_parameters():
                    if param.grad is not None:
                        print(f"Epoch {epoch} Batch {batch_idx} - Parameter '{name}' gradient mean: {param.grad.mean().item()}")
                    else:
                        print(f"Epoch {epoch} Batch {batch_idx} - Parameter '{name}' has no gradient!")

                optimizer.step()
                
                # # After optimization step, print updated parameters
                # print(f"Epoch {epoch} Batch {batch_idx} - After Optimization:")
                # for name, param in model.named_parameters():
                #     print(f"Parameter '{name}' value mean: {param.mean().item()}")

...
class FedAvg(object):
    ...
    def __init__(self, dataset, device, args, model_trainer):
        ...
        # !!!!!!
        self.ood_client = ClientAvg(-1, None, None, ood_data, len(ood_data.dataset), self.args, self.device, model_trainer)
    ...
    def _setup_clients(self, train_data_local_num_dict, train_data_local_dict, val_data_local_dict,
        ...
        for client_idx in range(self.client_num_in_total):
            c = ClientAvg(client_idx, train_data_local_dict[client_idx], val_data_local_dict[client_idx],test_data_local_dict[client_idx],
    ...
    def train(self):
        ...
        for round_idx in range(self.args.comm_round):
            ...
            for idx, client in enumerate(self.client_list):
                ...
                # train on new dataset# 使用全局模型参数在本地数据集上训练客户端模型，并返回训练后的本地模型参数。
                w = client.train(copy.deepcopy(w_global))
                # logging.info("local weights = " + str(w))# 将本地模型参数和样本数量添加到 w_locals 列表中
    ...
    # 测试方法 _ood_test_on_global_model 
    # 在全局模型上进行 OOD 测试
